...
class RasterProcessor {
    ...
    /**
     * Renderiza a máscara de mudanças como PNG TRANSPARENTE (RGBA), pronto para
     * ser sobreposto por cima de uma imagem real (satélite true-color) via
     * posicionamento absoluto em CSS. Pixels fora da máscara ficam 100%
     * transparentes; pixels dentro recebem a cor informada, com o contorno de
     * cada região destacado com opacidade maior para facilitar a leitura.
     * @param {Uint8Array} mask - 1 = pixel com alteração, 0 = normal
     * @param {number} width
     * @param {number} height
     * @param {Object} [opts]
     * @param {number} [opts.canvasWidth=width]
     * @param {number} [opts.canvasHeight=height]
     * @param {[number,number,number]} [opts.color=[255,59,59]] - RGB do destaque
     * @param {number} [opts.fillAlpha=110] - opacidade do preenchimento (0-255)
     * @param {number} [opts.borderAlpha=235] - opacidade do contorno (0-255)
     * @returns {HTMLCanvasElement}
     */
    renderMaskOverlayCanvas(mask, width, height, opts = {}) {
        const canvasWidth = opts.canvasWidth || width;
        const canvasHeight = opts.canvasHeight || height;
        const [r, g, b] = opts.color || [255, 59, 59];
        const fillAlpha = opts.fillAlpha ?? 110;
        const borderAlpha = opts.borderAlpha ?? 235;
        ...
        for (let py = 0; py < canvasHeight; py++) {
            for (let px = 0; px < canvasWidth; px++) {
                ...
                const isBorder = !isChanged(rx - 1, ry) || !isChanged(rx + 1, ry) ||
                                  !isChanged(rx, ry - 1) || !isChanged(rx, ry + 1);

                imgData.data[idx] = r;
                imgData.data[idx + 1] = g;
                imgData.data[idx + 2] = b;
                imgData.data[idx + 3] = isBorder ? borderAlpha : fillAlpha;
            }
        }
    ...
    /**
     * Wrapper que retorna direto o dataURL PNG (ver renderMaskOverlayCanvas).
     * @returns {string}
     */
    renderMaskOverlayPNG(mask, width, height, opts = {}) {
        return this.renderMaskOverlayCanvas(mask, width, height, opts).toDataURL('image/png');
    }
